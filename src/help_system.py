...
class HelpSystem:
    ...
    def _tutorial_welcome(self) -> bool:
        ...
        print(f"• How to get AI help for any command")
        print(f"• Using chat mode for complex questions")
        print(f"• Automatic error analysis and suggestions")
        print(f"• Configuration and customization")
        return True
    ...
    def _tutorial_configuration(self) -> bool:
        ...
        print(f"{self.ui.colorize('History file:', self.ui.Color.BRIGHT_WHITE, bold=True)} {self.ui.dim('~/.termsage/history')}")
    ...
    def get_contextual_tips(self, current_dir: str, recent_commands: list) -> list:
        ...
        if recent_commands:
            ...
            if "git" in last_cmd and not last_cmd.endswith("?"):
                tips.append(f"{self.ui.dim('Hint:')} Add {self.ui.command('?')} to git commands for help (e.g., {self.ui.command('git?')})")
